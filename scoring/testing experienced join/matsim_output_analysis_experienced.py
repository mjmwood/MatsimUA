...
def calculate_travel_utility(id, total_activities_modes, total_durations, total_distances, activity_indices, subpopulation, boardingTime, activity_ends, routes, tolls, stuck_id):
    ...
    for i in range(len(trips)):
        ...
        for j in range(len(trips[i])):
            ...
            try:
                tollcost = 0 #-1* assign_tolls(tolls, routes[i][j], trips[i][j]) #zero for example agents
                # print("would be tolls")
                print(id)
            except:
                print("tolls failed")
                print(id)
            ...
            STotal += STotal_temp
        STrans = 0 #transferCount * betaTrans #it appears MATSIM does not like to account for transfer penalties!
...
def calculate_activity_utility(stuck_bool, activity_modes, activity_indices, stuck_index, durations):
    # print(id)
    ...
    # betaEarly = 0
    # betaLate = 0
    betaPerf = 6 #6 for londinium and 10 for TE
    # betaWait = 0
    # betaShort = 0
    prio = 1
    ...
    if mode == "car":
        for link_id in route.split(' '):
            if link_id and link_id != "N/A":
                try:
                    toll_sum_temp += float(tolls[link_id])
                    # print("link found: "+str(link_id))
                except KeyError:
                    pass  # link not tolled
